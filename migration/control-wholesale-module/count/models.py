...
class PartnerMediaPrice(models.Model):
    """Private provider cost and public sale price for an API media plan."""
    ...
    duration = models.CharField(max_length=80, blank=True, default="", verbose_name="Duración")
    ...
    def __str__(self):
        return "{} · {}".format(self.get_service_display(), self.name)

    @property
    def profit(self):
        return self.sale_price - self.provider_cost

    @property
    def margin_percent(self):
        if not self.sale_price:
            return Decimal("0")
        return (self.profit / self.sale_price * Decimal("100")).quantize(
            Decimal("0.01")
        )
