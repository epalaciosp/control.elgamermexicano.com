...
def IndexView(request):
    ...
    if user_type == "superuser":
        ...
        my_services = CountsPackage.objects.filter(owner=request.user, saled=True)
        context.update({
            "my_services_total": my_services.count(),
            "my_renewals": my_services.filter(request_renewal=True).count(),
